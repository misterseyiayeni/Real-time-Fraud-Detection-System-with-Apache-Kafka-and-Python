...
# Create a SMTP session with Gmail's server
...
consumer = KafkaConsumer('transactions', bootstrap_servers = ['localhost:9092'], auto_offset_reset = 'earliest',
                         enable_auto_commit = True, group_id = 'fraudulent-transactions',
...
for message in consumer:
    ...
    if y_pred[0] == -1 or transaction['amount'] >= 9000:
        ...
        msg = EmailMessage()
        msg['From'] = "noreply@mail.com"
        msg['To'] = recipient_email
        # Set the subject
        msg['Subject'] = f"Important Notification about Transaction with id {transaction_id}!"
        # Set the message body with the name
        msg.set_content(f"The transaction with id {transaction_id} has been classified as fraudulent. Please review it "
                        f"immediately!")
